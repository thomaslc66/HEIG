...
/* those are taken from nachos.conf */
...
static void append_chunk(mem_chunk_t *chunk)
{
	...
	if (!quick_list) {
		...
		} else { /* Deively at the end of the list */
...
next_list:
	...
	while (victim && (victim->size < requested) && victim->next_list)
...
void *realloc(void *__ptr, size_t __size) {
	...
	char *_ptr = (char *) __ptr;
	void *alloc = malloc(__size);
	struct mem_chunk *chunk =  (struct mem_chunk *) (_ptr - sizeof(struct mem_chunk));

	DBG("Copying a size of %d\n", chunk->size);
	DBG("allocation pointer %p\n", alloc);

	memcpy(alloc, _ptr, chunk->size);
	free(__ptr);
